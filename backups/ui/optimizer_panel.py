...
class OptimizerPanel(QWidget):
    ...
    def _apply_settings(self):
        ...
        # Run advanced optimizations if requested
        if self.high_prio_checkbox.isChecked() or self.close_apps_checkbox.isChecked():
            ...
            if game_profile:
                self.ai_optimizer.run_system_optimizations(
                    game_profile=game_profile,
                    boost_priority=self.high_prio_checkbox.isChecked(),
                    close_apps=self.close_apps_checkbox.isChecked()
                )
